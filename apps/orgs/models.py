...
class Organization(models.Model):
    ...
    cies = models.ManyToManyField('users.User', related_name='related_cie_orgs', blank=True)
    ...
    def org_cies(self):
        from users.models import User
        if self.is_real():
            return self.cies.all()
        return User.objects.filter(role=User.ROLE_CIE)

    def get_org_auditors(self):
        return self.org_auditors()

    def get_org_cies(self):
        return self.org_cies()
    ...
    def get_org_members(self, exclude=()):
        ...
        if 'CIE' not in exclude:
            members |= self.get_org_cies()
    ...
    def can_cie_by(self, user):
        if user.is_super_cie:
            return True
        if self.get_org_cies().filter(id=user.id):
            return True
        return False
    ...
    @classmethod
    def get_user_cie_orgs(cls, user):
        cie_orgs = []
        if user.is_anonymous:
            return cie_orgs
        elif user.is_super_auditor:
            cie_orgs = list(cls.objects.all())
            cie_orgs.append(cls.default())
        elif user.is_org_cie:
            cie_orgs = user.related_cie_orgs.all()
        return cie_orgs
