...
class IsOrgCie(IsValidUser):
    """Allows access only to superuser"""

    def has_permission(self, request, view):
        if not current_org:
            return False
        return super(IsOrgCie, self).has_permission(request, view) \
            and current_org.can_cie_by(request.user)
...
class IsOrgAdminOrCieUser(IsValidUser):
    """Allows access between superuser and app user"""

    def has_permission(self, request, view):
        if not current_org:
            return False
        return super(IsOrgAdminOrCieUser, self).has_permission(request, view) \
            and (current_org.can_admin_by(request.user) or current_org.can_cie_by(request.user))
