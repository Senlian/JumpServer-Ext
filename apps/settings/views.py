...
from .forms import EmailSettingForm, LDAPSettingForm, BasicSettingForm, \
    TerminalSettingForm, SecuritySettingForm, EmailContentSettingForm, JenkinsSettingForm
...
class JenkinsSettingView(PermissionsMixin, TemplateView):
    form_class = JenkinsSettingForm
    template_name = "settings/jenkins_setting.html"
    permission_classes = [IsSuperUser]

    def get_context_data(self, **kwargs):
        context = {
            'app': _('Settings'),
            'action': _('Jenkins setting'),
            'form': self.form_class(),
        }
        kwargs.update(context)
        return super().get_context_data(**kwargs)
    ...
    def post(self, request):
        ...
        if form.is_valid():
            form.save()
            msg = _("Update setting successfully")
            messages.success(request, msg) # 结合bootstrap3的 {% bootstrap_messages %}
            return redirect('settings:jenkins-setting')
        else:
            context = self.get_context_data()
            context.update({"form": form})
            return render(request, self.template_name, context)